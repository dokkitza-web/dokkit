...
export function ProductInformationBox({
  ...
  return (
    <aside
      className={`border-l-4 border-[#ff6a00] bg-[#fff7f0] p-4 sm:p-5 ${className}`}
      aria-label="Product purchase information"
    >
      <p className="text-sm leading-6 text-[#3f3f43]">
        {PRODUCT_INFORMATION_COPY}
      </p>
      <div className="mt-3 flex flex-wrap gap-x-4 text-xs font-semibold">
        <Link
          href="/licence"
          className="inline-flex min-h-11 items-center text-[#005f73] underline underline-offset-4"
        >
          Template Licence
        </Link>
        <Link
