      ...
      <section className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
        <div className="grid gap-5 lg:grid-cols-3">
          {packageTiers.map((tier, index) => {
            const isBuyable = tier.key === "complete";
            ...
            return (
              ...
              >
                ...
                <div className="mt-6 grid gap-2">
                  <Link
                    href={isBuyable ? "/industries" : "/coming-soon"}
                    className={`inline-flex items-center justify-center rounded-md px-4 py-3 text-sm font-bold transition ${
                      isBuyable
                        ? "bg-[#f26a21] text-white hover:bg-[#d95816]"
                        : "bg-[#e5ddd5] text-[#7b746d]"
                    }`}
                  >
                    {isBuyable ? "Choose Complete Pack" : "Coming Soon"}
                  </Link>
