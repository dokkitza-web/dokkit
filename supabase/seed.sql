...
values
  ('quotation-template', 'Quotation Template', 'Editable quote layout for service and product businesses.', 'single_document', 7900, 1, 1, 1, '{"formats":["DOCX","XLSX","PDF"]}', true),
  ('invoice-workbook', 'Invoice Workbook', 'Simple invoice tracker with customer, item, tax, and payment fields.', 'single_document', 9900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('crm-tracker', 'CRM Tracker', 'Track leads, customers, follow-ups, status, and next actions.', 'single_document', 9900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('income-expense-tracker', 'Income and Expense Tracker', 'Monthly workbook for small-business income, expenses, and totals.', 'single_document', 9900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('terms-conditions-template', 'Terms and Conditions Template', 'Plain-language editable starting point for business terms.', 'single_document', 14900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('service-agreement', 'Service Agreement', 'Editable agreement for scope, fees, timelines, and responsibilities.', 'single_document', 14900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('client-onboarding-form', 'Client Onboarding Form', 'Capture customer details, needs, preferences, and approvals.', 'single_document', 6900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('sign-off-form', 'Sign-off Form', 'Confirm completion, acceptance, and customer comments.', 'single_document', 4900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('stock-tracker', 'Stock Tracker', 'Track stock received, sold, returned, and on hand.', 'single_document', 9900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('appointment-tracker', 'Appointment Tracker', 'Manage bookings, customer details, service type, and status.', 'single_document', 7900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('admin-checklist', 'Admin Checklist', 'Weekly and monthly admin checklist for small business owners.', 'single_document', 4900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('delivery-note', 'Delivery Note', 'Record items delivered, receiving person, date, and signature.', 'single_document', 4900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('job-card', 'Job Card', 'Capture job details, materials, labour, notes, and status.', 'single_document', 7900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('supplier-tracker', 'Supplier Tracker', 'Track supplier contact details, terms, orders, and performance.', 'single_document', 7900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('cash-up-sheet', 'Cash-up Sheet', 'Daily sales, cash, card, EFT, and variance tracking workbook.', 'single_document', 7900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('project-tracker', 'Project Tracker', 'Track project tasks, owners, deadlines, status, and notes.', 'single_document', 9900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('returns-form', 'Refund and Returns Form', 'Capture return reasons, refund decisions, and customer details.', 'single_document', 6900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('consent-form', 'Consent Form', 'Editable customer consent form for services that need approval.', 'single_document', 6900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true),
  ('monthly-dashboard', 'Monthly Business Dashboard', 'Workbook for sales, expenses, customers, tasks, and monthly notes.', 'single_document', 12900, 0, 1, 1, '{"formats":["XLSX","PDF"]}', true),
  ('customer-feedback-form', 'Customer Feedback Form', 'Collect ratings, comments, referrals, and improvement notes.', 'single_document', 4900, 1, 0, 1, '{"formats":["DOCX","PDF"]}', true)
on conflict (slug) do update set
  name = excluded.name,
  description = excluded.description,
  price_cents = excluded.price_cents,
